...
(function() {
	 ...
	 * 因此，UEditor提供了针对不同页面的编辑器可单独配置的根路径，具体来说，
	 * 在需要实例化编辑器的页面最顶部写上如下代码即可。当然，需要令此处的URL等于对应的配置。 window.UEDITOR_HOME_URL =
	 * "/xxxx/xxxx/";
	 */
	var URL;
	window.UEDITOR_HOME_URL = "/resources/third-party/ueditor/";
	/**
	...
	URL = window.UEDITOR_HOME_URL
			|| tmp.substr(0, tmp.lastIndexOf("\/") + 1).replace("_examples/",
					"").replace("website/", "");// 这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
	...
	window.UEDITOR_CONFIG = {
		...
		snapscreenServerUrl : URL + "jsp/imageUp.jsp" // 屏幕截图的server端保存程序，UEditor的范例代码为“URL
														// +"server/upload/jsp/snapImgUp.jsp"”
		,
		...
		toolbars : [ [ 'fullscreen', 'source', '|', 'undo', 'redo', '|',
				...
				'insertframe', 'highlightcode', 'webapp', 'pagebreak',
				'template', 'background', '|', 'horizontal', 'date', 'time',
				'spechars', 'snapscreen', 'wordimage', '|', 'inserttable',
				'deletetable', 'insertparagraphbeforetable', 'insertrow',
				'deleterow', 'insertcol', 'deletecol', 'mergecells',
				'mergeright', 'mergedown', 'splittocells', 'splittorows',
				'splittocols', '|', 'print', 'preview', 'searchreplace', 'help' ] ]
	...
	// , serialize : {
	// //黑名单，编辑器会过滤掉一下标签
	// blackList:{object:1, applet:1, input:1, meta:1, base:1, button:1,
	// select:1, textarea:1, '#comment':1, 'map':1, 'area':1}
	// }
	...
	// textAlign : "left" , //段落的排版方式，可以是 left,right,center,justify
	// 去掉这个属性表示不执行排版
